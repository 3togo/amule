...
bool CClientTCPSocket::ProcessPacket(const uint8_t *buffer, uint32 size, uint8 opcode)
{
	...
	case OP_ASKSHAREDDENIEDANS:
		AddDebugLogLineN(logRemoteClient,
			"Remote Client: OP_ASKSHAREDDENIEDANS from " + m_client->GetFullIP());
	...
	default:
		theStats::AddDownOverheadOther(size);
		AddDebugLogLineN(logRemoteClient,
			CFormat("Edonkey packet: unknown opcode: %i %x from %s") % opcode % opcode %
				m_client->GetFullIP());
		return false;
	}
...
bool CClientTCPSocket::ProcessExtPacket(const uint8_t *buffer, uint32 size, uint8 opcode)
{
	...
	*/
	...
	case OP_MULTIPACKET_EXT:
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_MULTIPACKET_EXT from " + m_client->GetFullIP());
	/* fall through */
	case OP_MULTIPACKET: {
		if (opcode == OP_MULTIPACKET)
			AddDebugLogLineN(logRemoteClient,
				"Remote Client: OP_MULTIPACKET from " + m_client->GetFullIP());
		...
		if (reqfile == NULL) {
			...
			if (!(reqfile != NULL && reqfile->GetFileSize() > PARTSIZE)) {
				AddDebugLogLineN(
					logRemoteClient, "Remote client asked for a non-shared file");
				file_not_found = true;
			}
		}
		...
		if (!file_not_found && reqfile->IsLargeFile() && !m_client->SupportsLargeFiles()) {
			AddDebugLogLineN(logRemoteClient,
				"Remote client asked for a large file but doesn't support them");
			file_not_found = true;
		}

		if (!file_not_found && nSize && (reqfile->GetFileSize() != nSize)) {
			AddDebugLogLineN(
				logRemoteClient, "Remote client asked for a file but specified wrong size");
			file_not_found = true;
		}
		...
		if (file_not_found) {
			...
			theStats::AddUpOverheadFileRequest(replypacket->GetPacketSize());
			AddDebugLogLineN(logLocalClient,
				"Local Client: OP_FILEREQANSNOFIL to " + m_client->GetFullIP());
			SendPacket(replypacket, true);
			break;
		}
		...
		while (data_in.GetLength() - data_in.GetPosition()) {
			if (!m_client) {
				throw wxString("Client suddenly disconnected");
			}
			uint8 opcode_in = data_in.ReadUInt8();
			switch (opcode_in) {
			case OP_REQUESTFILENAME: {
				AddDebugLogLineN(logRemoteClient,
					"Remote Client: OP_MULTIPACKET has OP_REQUESTFILENAME");
				m_client->ProcessExtendedInfo(&data_in, reqfile);
			...
			case OP_AICHFILEHASHREQ: {
				AddDebugLogLineN(logRemoteClient,
					"Remote Client: OP_MULTIPACKET has OP_AICHFILEHASHANS");
				if (m_client->IsSupportingAICH() &&
					reqfile->GetAICHHashset()->GetStatus() == AICH_HASHSETCOMPLETE &&
			...
			case OP_REQUESTSOURCES: {
				AddDebugLogLineN(logRemoteClient,
					"Remote Client: OP_MULTIPACKET has OP_REQUESTSOURCES(2)");
				uint8 byRequestedVersion = 0;
				uint16 byRequestedOptions = 0;
				if (opcode_in ==
					OP_REQUESTSOURCES2) { // SX2 requests contains additional data
					byRequestedVersion = data_in.ReadUInt8();
					byRequestedOptions = data_in.ReadUInt16();
				}

				// Although this shouldn't happen, it's a just in case to any Mods that mess
				// with version numbers.

				if (byRequestedVersion > 0 || m_client->GetSourceExchange1Version() > 1) {
					uint64 dwTimePassed = ::GetTickCount64() -
							      m_client->GetLastSrcReqTime() +
							      CONNECTION_LATENCY;
					...
					if (
						...
						CPacket *tosend = reqfile->CreateSrcInfoPacket(
							m_client, byRequestedVersion, byRequestedOptions);
						if (tosend) {
							theStats::AddUpOverheadSourceExchange(
								tosend->GetPacketSize());
							AddDebugLogLineN(logLocalClient,
								"Local Client: OP_ANSWERSOURCES to " +
									m_client->GetFullIP());
		...
		if (data_out.GetLength() > 16) {
			...
			AddDebugLogLineN(logLocalClient,
				"Local Client: OP_MULTIPACKETANSWER to " + m_client->GetFullIP());
	...
	case OP_MULTIPACKETANSWER: { // 0.43b
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_MULTIPACKETANSWER from " + m_client->GetFullIP());
		...
		if (!m_client->GetRequestFile()) {
			...
			throw wxString(" Wrong File ID: OP_MULTIPACKETANSWER; client->reqfile==NULL)");
		}
		if (reqfile != m_client->GetRequestFile()) {
			throw wxString(" Wrong File ID: OP_MULTIPACKETANSWER; reqfile!=client->reqfile)");
		}
		while (data_in.GetLength() - data_in.GetPosition()) {
	...
	case OP_EMULEINFO: { // 0.43b
		...
		if (!m_client->ProcessMuleInfoPacket(buffer, size)) {
			...
			AddDebugLogLineN(logRemoteClient, "Remote Client: OP_EMULEINFO is an OS_INFO");
		}
		break;
	}
	case OP_EMULEINFOANSWER: { // 0.43b
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_EMULEINFOANSWER from " + m_client->GetFullIP());
		theStats::AddDownOverheadOther(size);
	...
	case OP_PUBLICKEY: { // 0.43b
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_PUBLICKEY from " + m_client->GetFullIP());
	...
	case OP_SENDINGPART_I64:
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_SENDINGPART_I64 from " + m_client->GetFullIP());
	/* fall through */
	case OP_COMPRESSEDPART_I64:
		if (opcode == OP_COMPRESSEDPART_I64)
			AddDebugLogLineN(logRemoteClient,
				"Remote Client: OP_COMPRESSEDPART_I64 from " + m_client->GetFullIP());
	/* fall through */
	case OP_COMPRESSEDPART: { // 0.47a
		...
		if (m_client->GetRequestFile() && !m_client->GetRequestFile()->IsStopped() &&
			...
			m_client->ProcessBlockPacket(buffer,
				size,
				(opcode != OP_SENDINGPART_I64),
				(opcode == OP_COMPRESSEDPART_I64) || (opcode == OP_SENDINGPART_I64));
	...
	case OP_REQUESTPARTS_I64: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_REQUESTPARTS_I64 from " + m_client->GetFullIP());
	...
	case OP_QUEUERANKING: { // 0.43b
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_QUEUERANKING from " + m_client->GetFullIP());
		...
		if (size != 12) {
			throw wxString("Invalid size (OP_QUEUERANKING)");
		}
	...
	case OP_REQUESTSOURCES: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_REQUESTSOURCES from " + m_client->GetFullIP());
		...
		if (byRequestedVersion > 0 || m_client->GetSourceExchange1Version() >= 1) {
			...
			if (file) {
				...
				if (
					...
					(file->IsPartFile() &&
						static_cast<CPartFile *>(file)->GetSourceCount() <=
							RARE_FILE &&
						(bNeverAskedBefore || dwTimePassed > SOURCECLIENTREASKS)) ||
					// OR if file is not rare or if file is complete, allow every 90
					// minutes
					((bNeverAskedBefore ||
						dwTimePassed > SOURCECLIENTREASKS * MINCOMMONPENALTY))) {
					m_client->SetLastSrcReqTime();
					CPacket *tosend = file->CreateSrcInfoPacket(
						m_client, byRequestedVersion, byRequestedOptions);
					if (tosend) {
						theStats::AddUpOverheadSourceExchange(
							tosend->GetPacketSize());
	...
	case OP_ANSWERSOURCES: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_ANSWERSOURCES from " + m_client->GetFullIP());
		...
		if (file) {
			if (file->IsPartFile()) {
				...
				static_cast<CPartFile *>(file)->AddClientSources(&data,
					SF_SOURCE_EXCHANGE,
					m_client->GetSourceExchange1Version(),
					false,
					m_client);
			}
		}
	...
	case OP_FILEDESC: { // 0.43b
		AddDebugLogLineN(logRemoteClient, "Remote Client: OP_FILEDESC from " + m_client->GetFullIP());
	...
	case OP_REQUESTPREVIEW: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_REQUESTPREVIEW  from " + m_client->GetFullIP());
		break;
	}
	// Unsupported
	case OP_PREVIEWANSWER: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_PREVIEWANSWER from " + m_client->GetFullIP());
		break;
	}

	case OP_PUBLICIP_ANSWER: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_PUBLICIP_ANSWER from " + m_client->GetFullIP());
		theStats::AddDownOverheadOther(size);
	...
	case OP_PUBLICIP_REQ: {
		...
		AddDebugLogLineN(
			logLocalClient, "Local Client: OP_PUBLICIP_ANSWER to" + m_client->GetFullIP());
		SendPacket(pPacket);
		break;
	}
	case OP_AICHANSWER: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_AICHANSWER from " + m_client->GetFullIP());
		theStats::AddDownOverheadOther(size);
	...
	case OP_AICHFILEHASHANS: {
		...
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_AICHFILEHASHANS from " + m_client->GetFullIP());
		theStats::AddDownOverheadOther(size);
		CMemFile data(buffer, size);
	...
	case OP_AICHFILEHASHREQ: {
		// those should not be received normally, since we should only get those in MULTIPACKET
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_AICHFILEHASHREQ from " + m_client->GetFullIP());
		CMemFile data(buffer, size);
		CMD4Hash hash = data.ReadHash();
		...
		if (m_client->IsSupportingAICH() &&
			pPartFile->GetAICHHashset()->GetStatus() == AICH_HASHSETCOMPLETE &&
			...
			AddDebugLogLineN(logLocalClient,
				"Local Client: OP_AICHFILEHASHANS to" + m_client->GetFullIP());
			SendPacket(packet);
		}
		break;
	}
	case OP_CALLBACK: {
		AddDebugLogLineN(logRemoteClient, "Remote Client: OP_CALLBACK from " + m_client->GetFullIP());
		theStats::AddDownOverheadFileRequest(size);
	...
	case OP_BUDDYPING: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_BUDDYPING from " + m_client->GetFullIP());
		theStats::AddDownOverheadKad(size);

		CUpDownClient *buddy = theApp->clientlist->GetBuddy();
		if (buddy != m_client || m_client->GetKadVersion() == 0 ||
			!m_client->AllowIncomeingBuddyPingPong()) {
		...
		AddDebugLogLineN(logLocalClient, "Local Client: OP_BUDDYPONG to " + m_client->GetFullIP());
		SendPacket(replypacket);
		break;
	}
	case OP_BUDDYPONG: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_BUDDYPONG from " + m_client->GetFullIP());
		theStats::AddDownOverheadKad(size);
	...
	case OP_REASKCALLBACKTCP: {
		...
		if (buddy != m_client) {
			AddDebugLogLineN(logRemoteClient,
				"Remote Client: OP_REASKCALLBACKTCP from " + m_client->GetFullIP() +
					" which is not our buddy!");
		...
		if (!reqfile) {
			...
			if (sender) {
				...
				theApp->clientudp->SendPacket(
					response, destip, destport, false, NULL, false, 0);
			}
			break;
		}
		...
		if (sender) {
			...
			if (hash == sender->GetUploadFileID()) {
				...
				theStats::AddUpOverheadFileRequest(response->GetPacketSize());
				AddDebugLogLineN(logLocalClient,
					"Local Client UDP: OP_REASKACK to " + m_client->GetFullIP());
				theApp->clientudp->SendPacket(response,
					destip,
					destport,
					sender->ShouldReceiveCryptUDPPackets(),
				...
				AddDebugLogLineN(logListenSocket,
					"Client UDP socket; OP_REASKCALLBACKTCP; reqfile does not match");
			}
		} else {
			if (!bSenderMultipleIpUnknown) {
				if ((theStats::GetWaitingUserCount() + 50) > thePrefs::GetQueueSize()) {
					AddDebugLogLineN(logLocalClient,
						"Local Client: OP_QUEUEFULL to " + m_client->GetFullIP());
				...
				AddDebugLogLineN(logRemoteClient,
					CFormat("OP_REASKCALLBACKTCP Packet received - multiple clients with "
						"the same IP but different UDP port found. Possible UDP "
						"Portmapping problem, enforcing TCP connection. IP: %s, "
						"Port: %u") %
	...
	case OP_CHATCAPTCHAREQ: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_CHATCAPTCHAREQ from " + m_client->GetFullIP());
		theStats::AddDownOverheadOther(size);
		CMemFile data_in(buffer, size);
	...
	case OP_CHATCAPTCHARES: {
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_CHATCAPTCHARES from " + m_client->GetFullIP());
		theStats::AddDownOverheadOther(size);
		if (size) {
	...
	case OP_FWCHECKUDPREQ: { // Support required for Kadversion >= 6
		AddDebugLogLineN(
			logRemoteClient, "Remote Client: OP_FWCHECKUDPREQ from " + m_client->GetFullIP());
		theStats::AddDownOverheadOther(size);
		CMemFile data_in(buffer, size);
	...
	case OP_KAD_FWTCPCHECK_ACK: { // Support required for Kadversion >= 7
		AddDebugLogLineN(logRemoteClient,
			"Remote Client: OP_KAD_FWTCPCHECK_ACK from " + m_client->GetFullIP());
		if (theApp->clientlist->IsKadFirewallCheckIP(m_client->GetIP())) {
			...
			AddDebugLogLineN(logListenSocket,
				"Received unrequested OP_KAD_FWTCPCHECK_ACK packet from " +
					m_client->GetFullIP());
		}
		break;
	}
	default:
		theStats::AddDownOverheadOther(size);
		AddDebugLogLineN(logRemoteClient,
			CFormat("eMule packet : unknown opcode: %i %x from %s") % opcode % opcode %
				m_client->GetFullIP());
		break;
	}
...
bool CClientTCPSocket::ProcessED2Kv2Packet(const uint8_t *buffer, uint32 size, uint8 opcode)
{
	...
	if (!m_client) {
		throw wxString("Unknown clients sends extended ED2Kv2 protocol packet");
	}
	...
	try {
		...
		case OP_QUEUERANK: {
			AddDebugLogLineN(logRemoteClient,
				"Remote Client: ED2Kv2 OP_QUEUERANK from " + m_client->GetFullIP());
		...
		default:
			theStats::AddDownOverheadOther(size);
			AddDebugLogLineN(logRemoteClient,
				CFormat("ED2Kv2 packet : unknown opcode: %i %x from %s") % opcode % opcode %
					m_client->GetFullIP());
		}
	} catch (...) {
		AddDebugLogLineN(logRemoteClient,
			CFormat("ED2Kv2 packet is corrupt at pos %i! opcode: %i %x from %s") %
				data.GetPosition() % opcode % opcode % m_client->GetFullIP());
		throw;
	}
...
void CClientTCPSocket::OnReceive(int nErrorCode)
{
	...
	if (theApp->ipfilter->IsFiltered(m_remoteAddress)) {
		...
		Safe_Delete();
		AddDebugLogLineN(
			logIPFilter, "A connected client was dropped by IPFilter on new packet received");
	} else {
	...
	if ((nErrorCode == 0) || (nErrorCode == 7) || (nErrorCode == 0xFEFF)) {
		if (m_client) {
			if (!m_client->GetUserName().IsEmpty()) {
				strError = "Client '" + m_client->GetUserName() + "'";
			} else {
				strError = "An unnamed client";
			}
			strError += " (IP:" + m_client->GetFullIP() + ") ";
		} else {
			strError = "A client ";
		}
		if (nErrorCode == 0) {
			strError += "closed connection.";
		} else if (nErrorCode == 0xFEFF) {
			strError += " caused a wxSOCKET_LOST event.";
		} else {
			strError += "caused a socket blocking error.";
		}
	} else {
		if (theLogger.IsEnabled(logClient) && nErrorCode != 107) {
			...
			if (m_client) {
				if (!m_client->GetUserName().IsEmpty()) {
					strError = CFormat("OnError: Client '%s' (IP:%s) caused an error: "
							   "%u. Disconnecting client!") %
						   m_client->GetUserName() % m_client->GetFullIP() %
						   nErrorCode;
				} else {
					strError = CFormat("OnError: Unknown client (IP:%s) caused an error: "
							   "%u. Disconnecting client!") %
						   m_client->GetFullIP() % nErrorCode;
				}
			} else {
				strError = CFormat("OnError: A client caused an error or did something bad "
						   "(error %u). Disconnecting client !") %
					   nErrorCode;
			}
		} else {
			strError = "Error 107 (Transport endpoint is not connected)";
		}
	}
...
bool CClientTCPSocket::PacketReceived(CPacket *packet)
{
	...
	AddDebugLogLineN(logRemoteClient,
		CFormat("Packet with protocol %x, opcode %x, size %u received from %s") %
			packet->GetProtocol() % packet->GetOpCode() % packet->GetPacketSize() %
			(m_client ? m_client->GetFullIP() : "Unknown Client"));
	...
	try {
		...
		if ((packet->GetProtocol() == OP_PACKEDPROT) ||
			...
			if (!packet->UnPackPacket()) {
				AddDebugLogLineN(logZLib, "Failed to decompress client TCP packet.");
				bResult = false;
				process = false;
			} else {
				AddDebugLogLineN(logRemoteClient,
					CFormat("Packet unpacked, new protocol %x, opcode %x, size %u") %
						packet->GetProtocol() % packet->GetOpCode() %
		...
		if (process) {
			...
			case OP_EDONKEYPROT:
				bResult =
					ProcessPacket(packet->GetDataBuffer(), uRawSize, packet->GetOpCode());
				break;
			case OP_EMULEPROT:
				bResult = ProcessExtPacket(packet->GetDataBuffer(),
					packet->GetPacketSize(),
					packet->GetOpCode());
				break;
			case OP_ED2KV2HEADER:
				bResult = ProcessED2Kv2Packet(packet->GetDataBuffer(),
					packet->GetPacketSize(),
					packet->GetOpCode());
				break;
		...
		exception = "EOF exception: " + err.what();
	} catch (const CInvalidPacket &err) {
		exception = "InvalidPacket exception: " + err.what();
	} catch (const wxString &error) {
		exception = "error: " + (error.IsEmpty() ? wxString("Unknown error") : error);
	}
	...
	if (!exception.IsEmpty()) {
		AddDebugLogLineN(logPacketErrors,
			CFormat("Caught %s\nOn packet with protocol %x, opcode %x, size %u\tClientData: "
				"%s\n") %
				exception % packet->GetProtocol() % packet->GetOpCode() %
				packet->GetPacketSize() %
				(m_client ? m_client->GetClientFullInfo() : "Unknown"));
		...
		AddDebugLogLineN(logClient,
			CFormat("Client '%s' (IP: %s) caused an error (%s). Disconnecting client!") %
				(m_client ? m_client->GetUserName() : wxString("Unknown")) %
				(m_client ? m_client->GetFullIP() : wxString("Unknown")) % exception);

		Disconnect("Caught exception on CClientTCPSocket::ProcessPacket\n");
	}

	return bResult;
}

SocketSentBytes CClientTCPSocket::SendControlData(
	uint32 maxNumberOfBytesToSend, uint32 overchargeMaxBytesToSend)
{
	SocketSentBytes returnStatus =
		CEMSocket::SendControlData(maxNumberOfBytesToSend, overchargeMaxBytesToSend);

	if (returnStatus.success &&
		(returnStatus.sentBytesControlPackets > 0 || returnStatus.sentBytesStandardPackets > 0)) {
		ResetTimeOutTimer();
	}
...
SocketSentBytes CClientTCPSocket::SendFileAndControlData(
	uint32 maxNumberOfBytesToSend, uint32 overchargeMaxBytesToSend)
{
	SocketSentBytes returnStatus =
		CEMSocket::SendFileAndControlData(maxNumberOfBytesToSend, overchargeMaxBytesToSend);
...
void CClientTCPSocket::SendPacket(
