...
uint32_t PerSearchState::getProgress() const
{
	...
	if (m_searchType == GlobalSearch) {
		if (m_totalServerCount == 0) {
			return 0;
		}

		size_t queriedCount = m_queriedServers.size();
		uint32_t progress = 100 - (queriedCount * 100) / m_totalServerCount;

		AddDebugLogLineC(logSearch, CFormat(wxT("Progress for ID=%u: queried=%zu/%zu, progress=%u")) % m_searchId % queriedCount % m_totalServerCount % progress);
