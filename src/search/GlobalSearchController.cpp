...
#include "../search/SearchTypeConverter.h"  // Include for search type conversion
#include <wx/utils.h>  // Include for wxString::Format
...
void GlobalSearchController::startSearch(const SearchParams& params)
{
	...
	SEARCH_DEBUG_CONTROLLER(
		wxString::Format(wxT("GlobalSearchController: Generated search ID %u for global search: %s"), 
	...
	if (theApp && theApp->searchlist) {
		...
		// Store search parameters for later retrieval  
		theApp->searchlist->StoreSearchParams(searchId, legacyParams);

		SEARCH_DEBUG_CONTROLLER(
			wxString::Format(wxT("GlobalSearchController: Set current search ID %u in SearchList"), searchId));
	}
	...
	SearchResultRouter::Instance().RegisterTypeController(::GlobalSearch, this);
