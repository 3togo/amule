...
std::pair<uint32_t, wxString> ED2KSearchController::executeSearch(const SearchParams& params)
{
	...
	if (theApp && theApp->serverconnect) {
		...
		// Send the packet (for local search, SendPacket takes ownership;
		// for global search, the packet is owned by PerSearchState via unique_ptr)
		if (isLocalSearch) {
		theApp->serverconnect->SendPacket(packet, isLocalSearch);
		} else {
		// For global search, PerSearchState owns the packet via unique_ptr.
		// SendPacket would take ownership, but we've already transferred it.
		// Create a copy for SendPacket since PerSearchState retains ownership.
		CMemFile dataCopy(packetData, packetSize);
		CPacket* sendPacket = new CPacket(dataCopy, OP_EDONKEYPROT, OP_SEARCHREQUEST);
		theApp->serverconnect->SendPacket(sendPacket, false);
		}
