...
#include "UnifiedSearchEngineManager.h"
...
UnifiedSearchEngineManager::UnifiedSearchEngineManager(const Config& config)
    : m_config(config)
{
    m_statistics.startTime = std::chrono::system_clock::now();
}

UnifiedSearchEngineManager::UnifiedSearchEngineManager() : m_config(Config{})
{
    m_statistics.startTime = std::chrono::system_clock::now();
}

UnifiedSearchEngineManager::~UnifiedSearchEngineManager()
{
    Shutdown();
}

void UnifiedSearchEngineManager::Start()
{
    if (m_running.load()) {
    ...
    // Start worker thread
    m_workerThread = std::thread(&UnifiedSearchEngineManager::WorkerThread, this);

    std::cout << "[UnifiedSearchEngineManager] Started" << std::endl;
}

void UnifiedSearchEngineManager::Shutdown()
{
    if (!m_running.load()) {
        return; // Not running
    }
    ...
    std::cout << "[UnifiedSearchEngineManager] Shutdown complete" << std::endl;
}

bool UnifiedSearchEngineManager::IsRunning() const
{
    return m_running.load();
}

bool UnifiedSearchEngineManager::SendCommand(const SearchCommand& command)
{
...
void UnifiedSearchEngineManager::SetEventCallback(std::function<void(const SearchEvent&)> callback)
{
    std::lock_guard<std::mutex> lock(m_eventCallbackMutex);
    m_eventCallback = std::move(callback);
}

UnifiedSearchEngineManager::Statistics UnifiedSearchEngineManager::GetStatistics() const
{
    // Return a copy (safe since we're not modifying)
    return m_statistics;
}

void UnifiedSearchEngineManager::WorkerThread()
{
    std::cout << "[UnifiedSearchEngineManager] Worker thread started" << std::endl;
    ...
    std::cout << "[UnifiedSearchEngineManager] Worker thread stopped" << std::endl;
}

void UnifiedSearchEngineManager::ProcessCommand(const SearchCommand& command)
{
    try {
        switch (command.type) {
            case SearchCommand::Type::START_SEARCH: {
                ...
                if (m_cacheManager && m_config.enableSearchCache) {
                    ...
                    if (m_cacheManager->FindExistingSearch(command.params, existingSearchId)) {
                        std::cout << "[UnifiedSearchEngineManager] Reusing existing search: "
                        ...
                        progress.statusMessage = "Requesting more results from existing search";
                        SendEvent(SearchEvent::ProgressUpdate(existingSearchId, progress));
                        return;
                    }
                }
            ...
            default:
                std::cerr << "[UnifiedSearchEngineManager] Unknown command type: "
                          << static_cast<int>(command.type) << std::endl;
                break;
        }
    } catch (const std::exception& e) {
        std::cerr << "[UnifiedSearchEngineManager] Error processing command: " << e.what() << std::endl;
        SendEvent(SearchEvent::ErrorOccurred(command.searchId, e.what()));
    }
}

void UnifiedSearchEngineManager::SendEvent(const SearchEvent& event)
{
    std::lock_guard<std::mutex> lock(m_eventCallbackMutex);
    ...
    if (m_eventCallback) {
        ...
        evt->SetClientObject(new SearchEventClientData(serialized));
...
void UnifiedSearchEngineManager::PerformMaintenance()
{
...
void UnifiedSearchEngineManager::InitializeEngines()
{
    std::cout << "[UnifiedSearchEngineManager] Initializing search engines" << std::endl;
    ...
    if (m_config.enableSearchCache) {
        ...
        std::cout << "[UnifiedSearchEngineManager] Search cache manager initialized" << std::endl;
    }

    std::cout << "[UnifiedSearchEngineManager] Search engines initialized" << std::endl;
}

void UnifiedSearchEngineManager::CleanupEngines()
{
    std::cout << "[UnifiedSearchEngineManager] Cleaning up search engines" << std::endl;
    ...
    std::cout << "[UnifiedSearchEngineManager] Search engines cleaned up" << std::endl;
}

ISearchEngine* UnifiedSearchEngineManager::SelectEngine(SearchType type)
{
    switch (type) {
