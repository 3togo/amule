...
void LocalSearchEngine::PerformSearch(SearchId searchId, const SearchParams& params)
{
    ...
    CSharedFileList* sharedFiles = theApp->sharedfiles;
    size_t fileCount = sharedFiles->GetCount();
    for (size_t i = 0; i < fileCount; ++i) {
        const CKnownFile* file = sharedFiles->GetFileByIndex(static_cast<unsigned int>(i));
        if (!file) {
            continue;
        }
        ...
        result.fileName = file->GetFileName().GetRaw().ToStdString();
        ...
        if (file->GetMetaDataVer() > 0) {
            // Add file type
            std::string fileType = GetED2KFileTypeSearchTerm(GetED2KFileTypeID(file->GetFileName())).ToStdString();
            if (!fileType.empty()) {
                result.fileType = fileType;
            }

            // Add other metadata if available
            for (const CTag& tag : file->GetTags()) {
