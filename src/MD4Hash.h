...
public:
	...
	/**
	 * Returns true if the hash appears corrupted.
	 *
	 * Truncated packets can yield a hash where exactly one half
	 * (first or last 8 bytes) is zeroed, which is invalid for a
	 * genuine MD4 hash. A fully zeroed hash is simply empty and is
	 * reported via IsEmpty() instead.
	 */
